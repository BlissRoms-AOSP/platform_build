#!/usr/bin/env python
# Copyright (C) 2012-2013, The BlissRoms Project
#
...
page = 1
while not depsonly:
    githubreq = urllib.request.Request("https://api.github.com/users/BlissRoms/repos?per_page=200&page=%d" % page)
    add_auth(githubreq)
...
def get_from_manifest(devicename):
    try:
        lm = ElementTree.parse(".repo/local_manifests/bliss_manifest.xml")
        lm = lm.getroot()
    except:
...
def add_to_manifest(repositories, fallback_branch = None):
    ...
    f = open('.repo/local_manifests/bliss_manifest.xml', 'w')
    f.write(raw_xml)
    f.close()
